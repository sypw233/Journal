...
object DatabaseExportDebugger {
     ...
     * @return 环境检查结果摘要
     */
    fun checkExportEnvironment(
        context: Context,
        databaseName: String = "journal_database"
    ): String {
        ...
        try {
            ...
            logAndAppend(
                logBuilder,
                "应用外部存储目录: ${externalFilesDir?.absolutePath ?: "不可用"}"
            )
            ...
            if (exportDirExists) {
                val exportedFiles =
                    exportDir.listFiles()?.filter { it.isFile && it.name.endsWith(".db") }
                        ?: emptyList()
                ...
                exportedFiles.forEachIndexed { index, file ->
                    logAndAppend(
                        logBuilder,
                        "导出文件 #${index + 1}: ${file.name}, 大小: ${file.length() / 1024} KB"
                    )
                }
            }
        ...
        try {
            ...
            if (externalDir != null) {
                ...
                try {
                    ...
                    logAndAppend(
                        logBuilder,
                        "应用专属外部存储写入测试成功: ${externalTestFile.absolutePath}"
                    )
