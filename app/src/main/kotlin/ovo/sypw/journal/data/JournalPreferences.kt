...
class JournalPreferences(context: Context) {
    ...
    // AI设置键
    private val keyAIModel = "ai_model_type"
    private val keyUseHistoricalJournalsDefault = "ai_use_historical_journals_default"
    private val keyHistoricalJournalsCountDefault = "ai_historical_journals_count_default"
    private val keyMaxContentLength = "ai_max_content_length"
    private val keyDefaultPromptTemplate = "ai_default_prompt_template"
    private val keyShowAdvancedSettingsDefault = "ai_show_advanced_settings_default"
    ...
    fun getBackupLocation(): String {
        return sharedPreferences.getString(keyBackupLocation, "内部存储/Journal/备份")
            ?: "内部存储/Journal/备份"
    }
    ...
    fun setAIModel(modelType: String) {
        sharedPreferences.edit { putString(keyAIModel, modelType) }
    }

    fun getAIModel(): String {
        return sharedPreferences.getString(keyAIModel, "qwen-turbo") ?: "qwen-turbo"
    }

    fun setUseHistoricalJournalsDefault(enabled: Boolean) {
        sharedPreferences.edit { putBoolean(keyUseHistoricalJournalsDefault, enabled) }
    }

    fun isUseHistoricalJournalsDefault(): Boolean {
        return sharedPreferences.getBoolean(keyUseHistoricalJournalsDefault, true)
    }

    fun setHistoricalJournalsCountDefault(count: Int) {
        sharedPreferences.edit { putInt(keyHistoricalJournalsCountDefault, count) }
    }

    fun getHistoricalJournalsCountDefault(): Int {
        return sharedPreferences.getInt(keyHistoricalJournalsCountDefault, 5)
    }

    fun setMaxContentLength(length: Int) {
        sharedPreferences.edit { putInt(keyMaxContentLength, length) }
    }

    fun getMaxContentLength(): Int {
        return sharedPreferences.getInt(keyMaxContentLength, 500)
    }

    fun setDefaultPromptTemplate(template: String) {
        sharedPreferences.edit { putString(keyDefaultPromptTemplate, template) }
    }

    fun getDefaultPromptTemplate(): String {
        return sharedPreferences.getString(keyDefaultPromptTemplate, "") ?: ""
    }

    fun setShowAdvancedSettingsDefault(enabled: Boolean) {
        sharedPreferences.edit { putBoolean(keyShowAdvancedSettingsDefault, enabled) }
    }

    fun isShowAdvancedSettingsDefault(): Boolean {
        return sharedPreferences.getBoolean(keyShowAdvancedSettingsDefault, false)
    }

    // 获取AI设置
    fun getAISettings(): AISettings {
        return AISettings(
            modelType = getAIModel(),
            useHistoricalJournalsDefault = isUseHistoricalJournalsDefault(),
            historicalJournalsCountDefault = getHistoricalJournalsCountDefault(),
            maxContentLength = getMaxContentLength(),
            defaultPromptTemplate = getDefaultPromptTemplate(),
            showAdvancedSettingsDefault = isShowAdvancedSettingsDefault()
        )
    }

    // 更新AI设置
    fun updateAISettings(settings: AISettings) {
        setAIModel(settings.modelType)
        setUseHistoricalJournalsDefault(settings.useHistoricalJournalsDefault)
        setHistoricalJournalsCountDefault(settings.historicalJournalsCountDefault)
        setMaxContentLength(settings.maxContentLength)
        setDefaultPromptTemplate(settings.defaultPromptTemplate)
        setShowAdvancedSettingsDefault(settings.showAdvancedSettingsDefault)
    }
